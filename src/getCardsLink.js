...
const getCardsLink = async () => {
    ...
    for (let i = 0; i < arrBtnsModChar.length; i++) {
        arrBtnsModChar[i].addEventListener('click', async () => {
            ...
            const chars = await response.json() // tableau objects
            document.getElementById(
                'mod-image'
            ).src = `data:image/png;base64, ${chars[i].image}`
            document.getElementById('mod-name').innerHTML = chars[i].name
            document.getElementById('mod-shortDescription').innerHTML =
                chars[i].shortDescription
            document.getElementById('mod-description').innerHTML =
                chars[i].description
            document
                .getElementById('btn-mod')
                .addEventListener('click', async () => {
                    ...
                    const newImage = inputs[0].addEventListener(
                        'change',
                        async (ev) => {
                        ...
                        .addEventListener('click', async () => {
                            try {
                                ...
                                if ((image = !preview)) {
                                    ...
                                    const modCharWithImage = await fetch(
                                        `https://character-database.becode.xyz/characters/${chars[i].id}`,
                                        {
                                            method: 'PUT',
                                            headers: {
                                                'Content-Type':
            ...
            document
                .getElementById('btn-mod-delete')
                .addEventListener('click', async () => {
                    const deletedChar = await fetch(
                        `https://character-database.becode.xyz/characters/${chars[i].id}`,
                        {
                            method: 'DELETE',
                            headers: {
